...
const ADMIN_PASSWORD = "grillin2024";
...
// ── CUSTOMER VIEW ─────────────────────────────────────────────────────────────
function CustomerView({ menuItems, search, setSearch, onOrderPlaced, orders, onAccessClick }) {
  ...
  const deliveryShort = form.type==="Delivery" && total<DELIVERY_MIN;
  ...
  const CartBody = () => (<>
    ...
    {cart.length>0&&<div className="cfoot"><div className="ctotal"><span>Subtotal</span><span>₹{total.toFixed(0)}</span></div><div className="ctotal" style={{fontSize:".8rem",color:"var(--m)",fontWeight:500}}><span>Delivery</span><span>Free</span></div><div className="ctotal" style={{borderTop:"1px solid var(--b)",paddingTop:".5rem"}}><span>Total</span><span>₹{total.toFixed(0)}</span></div><button className="chkbtn" onClick={()=>{setDrawerOpen(false);setShowCheckout(true);}}>Checkout</button></div>}
  </>);
  ...
  return (<>
    ...
    {showCheckout&&!orderPlaced&&(<div className="mov"><div className="modal" style={{maxHeight:"90vh",overflowY:"auto"}}>
      ...
      {form.type==="Delivery"&&<div className="addr-block"><div className="addr-block-title">🏠 Delivery Address *{autoFilled&&<span className="autofill-tag">✓ Auto-filled from previous order</span>}</div>
        ...
        <div className="fg" style={{marginBottom:0}}><label className="fl">Landmark</label><input className="fi" value={form.addr.landmark} onChange={e=>setAddr("landmark",e.target.value)}/></div></div>}
      {deliveryShort&&<div className="del-warn">⚠️ Minimum delivery order is <strong>₹{DELIVERY_MIN}</strong> — add ₹{DELIVERY_MIN-total} more</div>}
      <div className="fg"><label className="fl">Special Requests</label><input className="fi" value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} placeholder="Any special instructions…"/></div>
      <div className="osb">{cart.map(i=><div key={i.key} className="osi"><span>{cleanName(i.name)}{i.variant?` (${i.variant})`:""} ×{i.qty}</span><span>₹{(i.unitPrice*i.qty).toFixed(0)}</span></div>)}<div className="ost"><span>Total</span><span>₹{total.toFixed(0)}</span></div></div>
...
function AdminLogin({ onLogin }) {
  ...
  const go=()=>pwd===ADMIN_PASSWORD?onLogin():setErr(true);
...
// ── ADMIN PANEL ───────────────────────────────────────────────────────────────
function AdminPanel({ menuItems, setMenuItems, orders, setOrders, onLogout }) {
  ...
  const del=async(id)=>{if(window.confirm("Delete this item?")){setMenuItems(p=>p.filter(i=>i.id!==id));await deleteMenuItem(id);}};
  ...
  return (<div className="adm">
    ...
    <div className="adm-inner">
          ...
          <div className="tscr"><table><thead><tr><th>Item</th><th>Category</th><th>Price (₹)</th><th>Status</th><th>Actions</th></tr></thead><tbody>
            {filtered.map(item=><tr key={item.id}>
              ...
              <td>
                ...
                <button className="tdel" onClick={()=>del(item.id)}>Delete</button>
      ...
      {tab==="bulk"&&<div className="bulk"><h2>Bulk Add Items</h2><p>One item per line: <code>Name, Category, Price, Description, Emoji</code></p>
        ...
        {bParsed.length>0&&<div className="bprev">{bParsed.map(p=><div key={p.line} className="bpi"><span className={p.ok?"pok":"perr"}>{p.ok?"✓":"✗"}</span><span>{p.ok?`${p.emoji} ${p.name} — ${p.category} — ₹${p.price}`:`Line ${p.line}: ${p.err}`}</span></div>)}</div>}
...
export default function GrillinApp() {
  ...
  useEffect(() => {
    async function loadData() {
      const [menu, ords] = await Promise.all([fetchMenuItems(), fetchOrders()]);
      setMenuItems(menu);
      setOrders(ords);
      setLoading(false);
    }
  ...
  if (page === "admin-login") return <><style>{S}</style><AdminLogin onLogin={()=>{setAuthed(true);setPage("admin");}}/></>;
  if (page === "admin") return <><style>{S}</style><AdminPanel menuItems={menuItems} setMenuItems={setMenuItems} orders={orders} setOrders={setOrders} onLogout={()=>setPage("customer")}/></>;

  return (<><style>{S}</style><CustomerView menuItems={menuItems} search={search} setSearch={setSearch} onOrderPlaced={handleOrderPlaced} orders={orders} onAccessClick={()=>setPage(authed?"admin":"admin-login")}/></>);
}
