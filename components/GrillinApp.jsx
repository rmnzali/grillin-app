...
// ── CUSTOMER VIEW ─────────────────────────────────────────────────────────────
function CustomerView({ menuItems, search, setSearch, onOrderPlaced, orders, settings }) {
  ...
  const placeOrder = async () => {
    const order = {id:Date.now(),num:orderNum,customer:form.name,phone:form.phone,
      ...
      type:form.type,notes:cartNotes||"",items:cart.map(c=>({name:cleanName(c.name),variant:c.variant,qty:c.qty,unitPrice:c.unitPrice})),
  ...
  const reset = () => { setOrderPlaced(false);setShowCheckout(false);setForm({phone:"",name:"",type:"Delivery",addr:{house:"",apartment:"",street:"",place:"",landmark:""},notes:""});setAutoFilled(false);setCartNotes(""); };
  ...
  const CartBody = () => (<>
    ...
    {cart.length>0&&<div className="cfoot">
      <div style={{marginBottom:".6rem"}}><input className="fi" style={{fontSize:".78rem",padding:".45rem .7rem"}} placeholder="Special instructions for the kitchen…" value={cartNotes} onChange={e=>setCartNotes(e.target.value)}/></div>
    ...
    <div className="layout">
      ...
      <div className="menu">
        ...
        <div className="contact-footer">
          ...
          <a href="/admin" className="access-footer" style={{marginTop:".75rem",borderTop:"none",padding:".5rem",display:"block",textDecoration:"none"}}>Restaurant Access</a>
        </div>
      </div>
...
function AdminPanel({ menuItems, setMenuItems, orders, setOrders, onLogout, settings, setSettings, adminPassword, setAdminPassword }) {
  ...
  return (<div className="adm">
    <div className="adm-nav"><img src={LOGO_SRC} alt="Grillin'"/><a href="/" className="lout" style={{textDecoration:"none"}}>Sign Out</a></div>
...
// ── ROOT APP ──────────────────────────────────────────────────────────────────
export default function GrillinApp({ initialPage = "customer" }) {
  ...
  return (<><style>{S}</style><CustomerView menuItems={menuItems} search={search} setSearch={setSearch} onOrderPlaced={handleOrderPlaced} orders={orders} settings={settings}/></>);
}
