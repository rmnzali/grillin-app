...
.zomato-add:hover{background:rgba(220,38,38,.08)}
.zomato-add:active{transform:scale(.95)}
...
.zomato-qty button:hover{background:rgba(255,255,255,.12)}
.zomato-qty button:active{background:rgba(255,255,255,.2)}
...
.chead h2{font-family:var(--font);font-size:1.05rem;font-weight:800}
.chead p{font-size:.72rem;color:var(--m);margin-top:2px}
.cbody{flex:1;overflow-y:auto;padding:.5rem .9rem}
.cempty{text-align:center;padding:2.5rem 1rem;color:var(--m);font-size:.83rem}
.cempty-icon{font-size:2.2rem;margin-bottom:.5rem;opacity:.4}
...
.contact-footer a{color:var(--a);text-decoration:none;font-size:.78rem;font-weight:500}.contact-footer a:hover{text-decoration:underline}
...
.dclose{position:absolute;top:.65rem;right:1rem;background:var(--s2);border:none;border-radius:50%;width:28px;height:28px;color:var(--m);cursor:pointer;font-size:.8rem;display:flex;align-items:center;justify-content:center}
...
.sbox p{color:var(--m);font-size:.83rem;margin-bottom:1.3rem;line-height:1.6}
.otag{font-size:1rem;font-weight:700;color:var(--a);margin-bottom:.9rem}
...
.abox h2{font-size:1.5rem;font-weight:800}
.abox p{color:var(--m);font-size:.83rem;margin-bottom:1.3rem;margin-top:.35rem}
...
function CustomerView({ menuItems, search, setSearch, onOrderPlaced, orders, onAccessClick, settings }) {
  ...
  const placeOrder = async () => {
    const order = {id:Date.now(),num:orderNum,customer:form.name,phone:form.phone,address:form.type==="Delivery"?fullAddress(form.addr):null,addr:form.type==="Delivery"?form.addr:null,type:form.type,notes:form.addr.landmark||"",items:cart.map(c=>({name:cleanName(c.name),variant:c.variant,qty:c.qty,unitPrice:c.unitPrice,itemNote:c.itemNote||""})),total,status:"New",placedAt:new Date()};
                ...
                <div key={item.id} className={`mcard ${!item.available?"unavail":item.outOfStock?"oos":""}`} onClick={()=>handleClick(item)}>
                  ...
                  <div className="mc-right">
                    {item.outOfStock?<span className="oos-badge">Out of Stock</span>:!item.available?null:
                      qty>0?(
                        <div className="zomato-qty">
                          <button onClick={e=>handleItemQtyChange(item,-1,e)}>−</button>
                          <span>{qty}</span>
                          <button onClick={e=>{e.stopPropagation();handleClick(item);}}>+</button>
                        </div>
                      ):(
...
// ── SETTINGS (delete protection hardcoded ON) ──
...
function AdminPanel({ menuItems, setMenuItems, orders, setOrders, onLogout, settings, setSettings, adminPassword, setAdminPassword }) {
  ...
  const addIt=async()=>{if(!newItem.name||!newItem.price)return;const item={...newItem,id:Date.now(),available:true,outOfStock:false};setMenuItems(p=>[...p,item]);setNewItem({name:"",category:cats[0]||"",price:"",description:"",emoji:""});setShowAdd(false);await insertMenuItem(item);};
  ...
  return (<div className="adm">
      ...
      {tab==="manage"&&<><div className="astats"><div className="asc"><div className="asl">Total</div><div className="asv">{menuItems.length}</div></div><div className="asc"><div className="asl">Available</div><div className="asv">{avail}</div></div><div className="asc"><div className="asl">Out of Stock</div><div className="asv">{outOfStockCount}</div></div><div className="asc"><div className="asl">Hidden</div><div className="asv">{menuItems.filter(i=>!i.available).length}</div></div></div>
        <div className="tw"><div className="thb"><h2>Menu Items</h2><div className="tfs"><select className="fsel" value={fCat} onChange={e=>setFCat(e.target.value)}><option value="All">All Categories</option>{cats.map(c=><option key={c}>{c}</option>)}</select><select className="fsel" value={fSt} onChange={e=>setFSt(e.target.value)}><option value="All">All Status</option><option>Available</option><option>Unavailable</option><option>Out of Stock</option></select><button className="abtn" onClick={()=>setShowAdd(true)}>+ Add</button></div></div>
          <div className="tscr"><table><thead><tr><th>Item</th><th>Category</th><th>Price</th><th>Status</th><th>Actions</th></tr></thead><tbody>
            {filtered.map(item=><tr key={item.id}><td><strong>{cleanName(item.name)}</strong>{item.description&&<div style={{fontSize:".7rem",color:"var(--m)",marginTop:2}}>{item.description}</div>}</td><td style={{color:"var(--m)",fontSize:".76rem"}}>{item.category}</td><td><strong style={{color:"var(--a)"}}>₹{item.price}</strong></td><td><span className={`bdg ${item.outOfStock?"boos":item.available?"bon":"boff"}`}>{item.outOfStock?"OOS":item.available?"On":"Off"}</span></td><td><button className="tedit" onClick={()=>setEditItem(item)}>Edit</button><button className="ttog" onClick={()=>tog(item.id)}>{item.available?"Hide":"Show"}</button><button className="ttog toos" onClick={()=>togOos(item.id)}>{item.outOfStock?"Restock":"OOS"}</button><button className="tdel" onClick={()=>requestDelete(item.id)}>Del</button></td></tr>)}</tbody></table></div></div></>}
    ...
    {showAdd&&<div className="mov"><div className="modal"><h2>Add Menu Item</h2><div className="fr"><div className="fg"><label className="fl">Name *</label><input className="fi" value={newItem.name} onChange={e=>setNewItem({...newItem,name:e.target.value})}/></div><div className="fg"><label className="fl">Emoji</label><input className="fi" value={newItem.emoji} onChange={e=>setNewItem({...newItem,emoji:e.target.value})}/></div></div><div className="fr"><div className="fg"><label className="fl">Category *</label><select className="fi" value={newItem.category} onChange={e=>setNewItem({...newItem,category:e.target.value})}>{cats.map(c=><option key={c}>{c}</option>)}</select></div><div className="fg"><label className="fl">Price *</label><input className="fi" value={newItem.price} onChange={e=>setNewItem({...newItem,price:e.target.value})}/></div></div><div className="fg"><label className="fl">Description</label><input className="fi" value={newItem.description} onChange={e=>setNewItem({...newItem,description:e.target.value})}/></div><div className="macts"><button className="bg" onClick={()=>setShowAdd(false)}>Cancel</button><button className="ba" onClick={addIt} disabled={!newItem.name||!newItem.price}>Add Item</button></div></div></div>}
...
function EditItemModal({ item, onSave, onClose }) {
  ...
  const save=()=>{if(!name.trim()||!price.trim())return;const base={...item,name:name.trim(),description:desc.trim(),price:price.trim(),emoji};if(popupType==="none"){delete base.popup;delete base.choices;delete base.addons;}if(popupType==="halfFull"){base.popup="halfFull";delete base.choices;delete base.addons;}if(popupType==="dryGravy"){base.popup="dryGravy";delete base.choices;delete base.addons;}if(popupType==="choices"){base.popup="choices";base.choices=choices.filter(c=>c.label.trim());delete base.addons;}if(popupType==="addon"){base.popup="addon";base.addons=choices.filter(c=>c.label.trim());delete base.choices;}onSave(base);};
...
export default function GrillinApp() {
  ...
  const [settings, setSettings] = useState({ deliveryMinEnabled:true, deleteProtection:true, offerEnabled:false, offerPercent:10 });
  useEffect(() => {
    async function loadData() {
      ...
      setSettings({ deliveryMinEnabled:dbSettings.delivery_min_enabled!=="false", deleteProtection:true, offerEnabled:dbSettings.offer_enabled==="true", offerPercent:parseInt(dbSettings.offer_percent)||10 });
      setLoading(false);
    }
    loadData();
  }, []);
